...
categories: ["homelab"]
preview: img-45f12335debc.png
feature: img-45f12335debc.png
---
...
Attached to this via a Netgear Managed Switch (I was playing about with VLANs and LAGG at this point) was a white box PC, using an AMD CPU, as they had more cores than Intel at the time. This was before the resurgence with Zen and was using ol' Bulldozer or Piledriver cores. The performance was *okay*, and overall, this was a great way to learn how to use ESX (which was handy for my first job).
...
I started running an actual "cluster" of VMware at home. I ran several applications, including playing around with [Apache Nifi](https://nifi.apache.org/), and [Plex](https://www.plex.tv/) became the new standard for home media. I connected to it via my XBOX One and set up [OpenVPN](https://www.openvpn.com/) to access it all remotely. This came in useful because the year after, I spent six months living in a hotel in Canberra.
...
![](img-018b2f8a2398.jpg)

## homelab v4: the allbids special (2018)

![](img-edc380f9b2a3.jpg)
...
![](img-a211e9f81c25.png)

![](img-a0e19ebc3509.png)

![](img-fb848b9d7d85.png)
...
*Side Note: In the rack picture, it appears there is a Dell R520... I did have one but for a very short amount of time. I think I intended to fill it with WD Green drives and run it as a shared storage solution for the R210iis. Again, power-wise, too much, and in summer (noting this was all in my garage), it did not fare well.*
...
![](img-1aeb0d548a4d.png)

>  *I got bored*

*Side Note: If you didn't know, my dog is called Zero. I have a thing for Nightmare before Christmas. It definitely doesn't show.*

*Side Side Note: In between publishing this post, and the next one, I've made changes. They will be detailed further in the next post on hardware choices*
...
- 1 x Disk Array - Synology DX1215
  - 10 x 10TB Seagate Ironwolf HDD in RAID6
  - 2 x 4TB PNY CS900 SSD in RAID1
...
- 3 x Compute Host - HP Elitedesk 800 G4 *New addition since publish*
  - Intel i5 8500T
  - 48Gb DDR4 RAM
  - 256Gb NVMe SSD
  - 500Gb NVMe SSD
...
I can't promise this will stay this way for long - I'm looking at swapping the M910q for a M920q, maybe some more compute nodes - but it's where we are now. I'm running [Proxmox](https://www.proxmox.com/en/) on each compute and services host, and [OPNSense](https://opnsense.org/) on the Gateway. You may think 16Gb in a gateway is overkill... and you're probably right, but I've also got [Suricata](https://suricata.io/), [bind](https://www.isc.org/bind/), and [Tailscale](https://tailscale.com/) as a subnet router running on it... but we're getting ahead of ourselves, that's a future part :). Also, I had it spare after I upgraded the other nodes.
...
The services host is my attempt at having a "prod" versus "dev": "prod" is made up of the docker containers on the NAS and the services host and will be anything that needs to be "on" permanently - Plex, [Vault](https://www.vaultproject.io/), [InfluxDB](https://www.influxdata.com/), all of that. The Compute Host cluster is an entirely destroyable and rebuildable system for me to evaluate products and services like [Istio](https://istio.io/), [Elastic](https://elastic.co/) and its agent, and [MISP](https://www.misp-project.org/). The NAS will provide storage to all of it, and I now have 2 x 4TB Drives in RAID1 to provide SMB storage to Proxmox and the Kubernetes cluster for persistent storage.
